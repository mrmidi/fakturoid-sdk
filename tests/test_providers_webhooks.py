...
from fakturoid_sdk.providers import WebhooksProvider
...
async def test_list() -> None:
    ...
    provider = WebhooksProvider(dispatcher)
...
async def test_get() -> None:
    ...
    dispatcher.get.assert_awaited_once_with("/accounts/{accountSlug}/webhooks/6.json")
...
async def test_delete() -> None:
    ...
    assert response.get_body(return_json_as_dict=True) == {"page": 2}
    dispatcher.delete.assert_awaited_once_with("/accounts/{accountSlug}/webhooks/6.json")
...
async def test_update() -> None:
    ...
    dispatcher.patch.assert_awaited_once_with(
        "/accounts/{accountSlug}/webhooks/6.json",
        {"page": 2},
    )
